...
$(function() {
	...
	if(optType == 1 || optType == 2){
        ...
        //项目信息
        Utils.ajaxJson(basePath+'/project/getProjectSummary',{projectId:$("#projectId").val()},function(obj){
	...
	$("#savePurReq").click(function() {
        //物料变更必须填写说明
        if (typeCode == "采购分包（物料变更）" && $("#remark").val() == "") {
...
//					}
				...
				},
				{
					...
					formatter : function(value, row, index) {
						if(optType != 3){
							var str = "";
							if (value != null) {
								str += '<span onclick="budgetBtn();" title=\"'
										+ value
										+ '\" class=\"easyui-tooltip\">'
										+ value + '</span>';
							}
				...
				},
				{
					...
					styler: function(value,row,index){
						if ((row.ISCOMEFROMPBS == 0 || row.MAXQTY == null || value > row.MAXQTY || value < 0) && !Utils.containsByArr(notProjectIntergration,typeCode)){
...
//						}
			...
			if (typeCode == "采购分包" || typeCode == "采购分包（物料变更）" || Utils.containsByArr(notProjectIntergration,typeCode)) {
				...
				if (typeCode == "采购分包（物料变更）") {
                    qtyCol.editor = {
                        type : "validatebox",
                        options : {
                            validType : [ "integer", "qty" ],
                            required : true
                        }
                    };
				}else {
...
//				editIndex = index;
	...
	if (typeCode == "采购分包" || typeCode == "采购分包（物料变更）") {
		...
		if (optType == 2) {
            ...
            $("#purreqlineRel_btn").click(function() {
                saveLine.click();
                if ($("#lineTable").datagrid('getEditors',
                    editIndex).length < 1) {
                    $('#reldlg').dialog({
...
function commitPurReq(){
	...
	if(purType==='采购分包' || purType==='采购分包（物料变更）' ){
		target = "P";
...
function loadPurReqData() {
    Utils.ajaxJson(basePath + "/purReq/getPurReq", {
        projectId : $("#projectId").val(),
        key : $("#reqeKey").val()
    }, function(obj) {
        var data = [];
        var constType = obj.rows;
        for (var i = 0; i < constType.length; i++) {
        	if (constType[i].TYPEID === '11111' && constType[i].STATUS === 2) {
                var d = {};
                d['id'] = constType[i].ID;
                d['text'] = constType[i].NAME + '('+constType[i].CODE+')';
                data.push(d);
			}
        }
        $("#reqTree").tree("loadData",data);
    });
}

function loadRelPurLine() {
    $("#lineList").datagrid({
        width: '100%',
        height: '100%',
        border: true,
...
//添加关联请购行
function addPueReqLineRel() {
	var line = $("#lineTable").datagrid("getSelected");
	if (line == null) {
        MyMessager.alert.show("提示","请选择请购行！");
        return;
	}
	var relLine = $("#lineList").datagrid("getSelected");
	if (relLine == null) {
        MyMessager.alert.show("提示","请选择关联的请购行！");
        return;
	}
	var data = {
		"lineId":line.ID,
		"relLineId":relLine.ID
	};
    $.messager.progress({
        interval : 100,
        text : '正在处理中...'
    ...
    $.ajax({
        url:basePath+'/purReq/addPurReqLineRel',
        type:'POST',
        data:data,
        success:function(obj){
            $.messager.progress("close");
        	if (obj.valid) {
                ...
                $.messager.alert("提示", "请选择相同的物料进行关联！");
			}
        	$("#purreqlineRel").datagrid("reload");
        },
        error: function(err){
            $.messager.progress("close");
            $.messager.alert("提示", "添加错误！");
        }
    });
}
